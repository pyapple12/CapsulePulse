...
**当前状态**：PL001–PL003（玻璃计时闭环 / 存储统计 / 提醒设置，V0.1.0.1–3）、首轮审计及其修复（A001/FIX001，V0.1.0.5）、运行时数据落址热更新（V0.1.0.6：config.json→configs/、pulse.db→data/，dev=项目根、release=exe 同级，杜绝机器用户目录）、PL004 托盘常驻与全局快捷键（V0.1.0.7：关闭隐藏到托盘 + 托盘菜单 + Alt+Shift+P/S 全局热键 + 单实例 + 退出落库）、PL005 工作日/打卡模型与统计视图（V0.1.0.8：workdays/events 两表 + 打卡双向确认框 + 未上班禁用计时 + 重启恢复在岗 + 自动下班回填记账 + 统计视图双标签/时间图谱/三值）均已完结——77 项测试全绿；macOS/Linux 延后 [problems#1]，通知署名随打包解决 [problems#2]，自动下班文案条启动竞态窗口 [problems#4]。PL006 苹果玻璃 UI 重设计·Liquid Glass（V0.1.0.9：全局设计令牌 + 唯一玻璃配方 blur28+saturate1.6+高光内描边 + 三档同心圆角 + 分段滑块 + 56px 主数字 + 浮层化弹层 + 胶囊条图谱；纯前端零 Rust 改动零新依赖）、第 2 轮全量审计修复（A002/FIX002，V0.1.0.10：pause 落库失败回滚 + storage 事务化 + 自动下班段末回填口径 + 读命令 async 化 + diag 文件日志 + 前端失败可见反馈）均已完结——89 项测试全绿（含 1 项存储探针集成测试）；macOS/Linux 延后 [problems#1]，通知署名随打包解决 [problems#2]，自动下班文案条启动竞态窗口 [problems#4]。PL007 糖果玻璃材质层（V0.1.0.11：糖果令牌换血——紫/薄荷 pastel 色板 + 轮廓光/带色投影/虹彩渐变五要素 + 按钮/分段滑块/图谱/三值 chips/数字渐变描字/浮层全面蒙皮 + Acrylic 浅暖紫 tint（唯一 Rust 触点一行常量））已完结——89 项测试全绿、C4 对比度探针 16/16（渐变描字达标未启用回退）；C1 双主题观感经主会话视觉核验通过（浅色随后续 PL 复核），macOS/Linux 延后 [problems#1]，实机运行期锁中毒已落取证（panic 钩子 + 锁名文案，根因待复现）[problems#5]。PL008 布局翻新（V0.1.0.12：380×560 可缩放窗 + min 夹取、底部 dock 导航（lucide-vue-next）、8h 进度环表盘（环心 40px 描字数字，未上班虚线置灰）、统计五卡纵流（导航/图谱/三值/周/明细，明细卡内滚）、Rust 新命令 week_detail + 跨周边界用例；pill 迁入 TimerCard、App 在岗态轮询收敛）已完结——90 项测试全绿、L1–L5 live 验证过（L1 极值拖拽部分验证，min 夹取 + 弹性布局保证）；浅色主题随日常/后续 PL 复核。PL009 光与生命感（V0.1.0.13：指针跟随高光（rAF 节流写 --mx/--my + 材质类 radial 高光层）、虹彩面 8s 环境光呼吸、微交互三件（按钮涟漪/打卡扫光/dock 切页光轨）、全部尊重 reduced-motion；three.js 时间盒结论 = 60fps 可行但不引入主线——折射只能作用于画布合成内容）已完结——90 项测试全绿、M1 实证过、M2/M3 调整验证（用户禁止改电脑系统设置验证，已登记「素材与环境陷阱」——代码走查 + 令牌机制确认替代）。PL010 材质重构·真实玻璃（V0.1.0.14：材质路线三易其稿终定 DWM 系统背板——Mica（暗色≈不透，弃）、采集式自绘（被遮挡像素物理不可采，1:1 直出实测 2-3fps 递归镜像，弃）→ **DWMWA_SYSTEMBACKDROP_TYPE=DWMSBT_TRANSIENTWINDOW**（微软终端同款，extern dwmapi 直连零新依赖，window-vibrancy 移除）：聚焦态真磨砂实证（壁纸透板、DWM 实时合成零延迟）+ 全窗单层收敛（margin 0、radius 8px、10% 统一薄纱）+ 拖拽全窗恢复（全局 mousedown + startDragging 白名单接线）+ 浅色高透配方表冻结落地；**失焦态实证为 DWM 材质焦点绑定边界——回退不透明灰板、无受支持 API 可拦**（终端 1.19 失焦保磨砂走 WinUI 私有通道，WebView2 无等价物），如实登记不宣布达成）已完结——90 项测试全绿（采集管线 4 测试随路线废弃移除）、用户实机判定通过。PL011 焦点联动材质·平时透明聚焦磨砂（V0.1.1.1：版本推进 0.1.0 → 0.1.1——平时纯 alpha 透明常驻（失焦不变），聚焦瞬间挂 DWM Acrylic 真磨砂（DWMSBT_TRANSIENTWINDOW=3）、失焦即刻撤回（**DWMSBT_NONE=1，非 0——0 是 AUTO**）；分态纱浓度（用户拍板）：透明态 30% 纱（浅白/纯黑，暗紫废弃）、磨砂态 0% 纱；window-focus 事件 + focused class + isFocused 兜底联动管线）已完结——90 项测试全绿、用户目验通过（CUA 中断期间 PowerShell 自动探针因前台锁/UIPI 废弃，探针留档 .temp/）。第 3 轮全量审计及其修复（A003/FIX003，V0.1.1.2：拖拽白名单补 .overlay 修点外关闭回归 + 死代码清理（last_fired 隔离 / 删 DutySpan / storage 三方法隔离 / 删 --r-ctrl 孤儿变量）+ restart 留痕失败回滚（锚测试当场抓出 guard 重取自锁死锁并修复）+ 周卡失败可见性 + warn_diag 双写助手（关键失败路径 release 落档）+ 容错白名单第 ⑦ 项登记 + 五处过时注释更正）已完结——91 项测试全绿、storage_probe.rs 维持保留（A002-O4 豁免继续）。观察项全量整理（V0.1.1.3：三轮审计观察项去重归并 z.plan 第四节——永久豁免 21 项 / 条件豁免 13 项按触发条件分组（打包 5 / 跨平台 2 / 性能 3 / 需求与理论 3），失效销项 3（vibrancy 豁免 / restart 双锁 / eprintln 半收敛），A002/A003 观察项平行副本删除留归并指引；附带修复：settings.rs 临时文件清理失败补 diag 双写、tsconfig 开启 noUnusedLocals/noUnusedParameters（零爆出））。方案见 `z.plan.md` 附录，任务档案见 `x.progress.md`。`.agents/skills/` 存放项目自建 skill（audit-project / audit-report / progress-task）。遗留与远期项登记 `y.problems.md`。
...
- **验证禁止更改电脑系统设置**：live 验证一律以系统当前态进行（当前 = Windows 深色主题 + 动画效果开）；禁止为验证去切换主题、动画效果（reduced-motion）等系统设置——替代手段 = 代码走查确认退避/令牌规则在位 + 用户日常使用复核（2026-09-13 用户定案，M2/M3 类验证照此执行）
- **dev 无 HMR**：`beforeDevCommand = npm run build`、`frontendDist = dist`（静态产物）——改前端后必须重新 build + 重启 dev 流程才生效，热更不存在（2026-09-13 PL008 实测）
